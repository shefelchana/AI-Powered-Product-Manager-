...
export function pickWordOfDay(words, { lastLessonId = null, reviewedIds = new Set(), today }) {
  ...
  const lesson = pool.filter((w) => lastLessonId != null && w.lessonId === lastLessonId && !reviewedIds.has(w.id)).sort(byOldest);
  if (lesson.length > 0) return { word: lesson[0], reason: REASONS.lesson };

  const never = pool.filter((w) => !reviewedIds.has(w.id)).sort(byOldest);
